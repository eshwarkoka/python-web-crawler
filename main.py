        ...
        for link in links:
            ...
            if database.is_crawled(cursor=CURSOR, link=link):
                ...
                if database.check_if_link_crawled_before_24hrs(cursor=CURSOR, link=link):
                    # if True, do nothing!
                    print("Link is already crawled in the last 24 hrs...")
                    pass
                else:
                    # link is not crawled in last 24 hrs
                    print("Link is not crawled in the last 24 hrs...")
                    parser = 'html.parser'
                    ...
                    if response_status_code == 200:
                        # write response text to a local file
                        file_name = ''.join(i for i in link if i.isalnum())
                        with open('files/'+file_name+'.txt', 'w+') as f:
                            f.write(response_text)
                        file_path = str(os.getcwd())+'/files/'+str(file_name)+'.txt'
                        print("file created...")
                ...
                print("Link is not crawled at all...")
                parser = 'html.parser'
                response = requests.get(link)
                response_status_code = response.status_code
                response_text = response.text
